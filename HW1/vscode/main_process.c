...
void main_process()
{
    ...
    char key_buf[4], val_buf[16], val_input_buf[1000];
    ...
    while (1)
    {
        ...
        case MERGE_REQ:
        {
            ...
            if (dir == NULL)
            {
                perror("?îî?†â?Ü†Î¶¨Î?? ?ó¥ ?àò ?óÜ?äµ?ãà?ã§");
                return 1;
            }
            ...
            {
                // '.'?úºÎ°? ?ãú?ûë?ïò?äî ?ï≠Î™? (., .., ?à®Íπ? ?åå?ùº ?ì±) ?†ú?ô∏
                if (entry->d_name[0] != '.')
                {
                    fileCount++;
                }
            ...
            if (fileCount == 1)
            {
                memcpy(output->lcd2, "ONLY ONE FILE   ", 16);
            }
            else if (fileCount == 2)
            {
                ...
                shmdt(merge);
                char str[16];
                sprintf(str, "2 %d                      ", countLinesInFile("storage_files/2.st"));
                memcpy(output->lcd2, str, 16);
            }
...
void main_flush(struct table_elem *mem_table, int mem_table_cnt, bool _BACK_, bool _CALL_)
{
    ...
    if (!_CALL_)
    {
        const char *dirPath = "storage_files"; // ?îî?†â?Ü†Î¶? Í≤ΩÎ°ú
        char filePath[256];                    // ?åå?ùº Í≤ΩÎ°úÎ•? ????û•?ï† Î∞∞Ïó¥
        FILE *file;
        DIR *dir;
        struct dirent *entry;
        ...
        {
            printf("1.st, 2.st, 3.st ?åå?ùº?ù¥ Î™®Îëê Ï°¥Ïû¨?ï©?ãà?ã§. Ï∂îÍ?? ?åå?ùº ?Éù?Ñ±?ïòÏß? ?ïä?ùå.\n");
            return;
        }
        ...
        fclose(file);
        printf("%s ?åå?ùº?ù¥ ?Éù?Ñ±?êò?óà?äµ?ãà?ã§.\n", filePath);
    }
    int merge_q = shmget(MERGE_KEY, MERGE_MSG_SIZE, 0666 | IPC_CREAT);
    struct merge_msg *merge = (struct merge_msg *)shmat(merge_q, NULL, 0);
    merge->_BACK_ = _BACK_;
...
bool findValueByKey(const char *filename, int searchKey, char *result)
{
    ...
    if (!file)
    {
        perror("?åå?ùº ?ó¥Í∏? ?ã§?å®");
        strcpy(result, ""); // ?åå?ùº?ùÑ ?ó¥ ?àò ?óÜ?äî Í≤ΩÏö∞, Îπ? Î¨∏Ïûê?ó¥?ùÑ Î¶¨ÌÑ¥
        return;
    }

    int maxOrder = -1;       // ?òÑ?û¨ÍπåÏ?? Î∞úÍ≤¨?êú Í∞??û• ?Å∞ order Í∞íÏùÑ ????û•. Ï¥àÍ∏∞Í∞íÏ?? -1Î°? ?Ñ§?†ï?ïò?ó¨ ?ïÑÏß? Î∞úÍ≤¨?êòÏß? ?ïä?ïò?ùå?ùÑ ?ùòÎØ?
    char tempResult[5] = ""; // ?ûÑ?ãúÎ°? Í∞íÏùÑ ????û•?ï† Î∞∞Ïó¥
    bool found = false;
    ...
    while (!feof(file))
    {
        ...
        if (fscanf(file, "%d %d %s", &order, &key, value) == 3)
        {
            ...
            {
                strcpy(tempResult, value); // ?ÉàÎ°úÏö¥ ÏµúÎ?? order Í∞íÏùÑ Ï∞æÏïò?úºÎØ?Î°? tempResult?óê Í∞íÏùÑ Î≥µÏÇ¨
                maxOrder = order;          // maxOrderÎ•? ?óÖ?ç∞?ù¥?ä∏
                found = true;
            }
        }
    }
    ...
    if (found)
    {
        strcpy(result, tempResult); // Ï∞æÏ?? Í≤ΩÏö∞, ÏµúÏ¢Ö Í≤∞Í≥ºÎ•? result?óê Î≥µÏÇ¨
    }
    else
    {
        strcpy(result, "ERROR"); // Ï∞æÏ?? Î™ªÌïú Í≤ΩÏö∞, Îπ? Î¨∏Ïûê?ó¥?ùÑ Î¶¨ÌÑ¥
    }
...
{
    FILE *file = fopen(fileName, "r"); // ∆ƒ¿œ¿ª ¿–±‚ ∏µÂ∑Œ ø≠±‚
    if (file == NULL)
    {
        perror("∆ƒ¿œ¿ª ø≠ ºˆ æ¯Ω¿¥œ¥Ÿ");
        return -1; // ∆ƒ¿œ ø≠±‚ Ω«∆– Ω√ -1 π›»Ø
    }
    ...
    while (!feof(file))
    {
        ch = fgetc(file); // ∆ƒ¿œø°º≠ «— πÆ¿⁄ ¿–±‚
        if (ch == '\n')
        {
            lines++; // ¡ŸπŸ≤ﬁ πÆ¿⁄∏¶ ∏∏≥Ø ∂ß∏∂¥Ÿ ∂Û¿Œ ºˆ ¡ı∞°
        }
    }

    fclose(file); // ∆ƒ¿œ ¥›±‚
    return lines; // √— ∂Û¿Œ ºˆ π›»Ø
}