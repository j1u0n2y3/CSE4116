...
void main_flush(struct table_elem *mem_table, int mem_table_cnt, bool _BACK_, bool _CALL_){
    ...
    if(!_CALL_){
        const char *dirPath = "storage_files"; // 디렉토리 경로
        char filePath[256]; // 파일 경로를 저장할 배열
        FILE *file;
        DIR *dir;
        struct dirent *entry;
        ...
        if (!file1Exists) {
            ...
            printf("1.st, 2.st, 3.st 파일이 모두 존재합니다. 추가 파일 생성하지 않음.\n");
            return;
        }
...
void main_get(char *key_buf, char *val_buf, struct table_elem *mem_table){
    ...
    if(findValueByKey("storage_files/2.st", key, val_buf))return;
...
bool findValueByKey(const char *filename, int searchKey, char *result) {
    ...
    if (!file) {
        perror("파일 열기 실패");
        strcpy(result, ""); // 파일을 열 수 없는 경우, 빈 문자열을 리턴
        return;
    }

    int maxOrder = -1; // 현재까지 발견된 가장 큰 order 값을 저장. 초기값은 -1로 설정하여 아직 발견되지 않았음을 의미
    char tempResult[5] = ""; // 임시로 값을 저장할 배열
    bool found = false;
    ...
    while (!feof(file)) {
        ...
        if (fscanf(file, "%d %d %s", &order, &key, value) == 3) {
            if (key == searchKey && order > maxOrder) {
                strcpy(tempResult, value); // 새로운 최대 order 값을 찾았으므로 tempResult에 값을 복사
                maxOrder = order; // maxOrder를 업데이트
                found = true;
            }
        }
    }
    ...
    if (found) {
        strcpy(result, tempResult); // 찾은 경우, 최종 결과를 result에 복사
    } else {
        strcpy(result, "ERROR"); // 찾지 못한 경우, 빈 문자열을 리턴
    }
